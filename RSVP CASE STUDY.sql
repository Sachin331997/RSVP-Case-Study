...
SELECT Count(movie_id) AS movie_with_single_genre
FROM genre_summary; 
...
SELECT *,
       ...
       SUM(avg_duration) over (ORDER BY avg_duration ROWS 1 preceding) AS moving_avg_duration
       FROM genre_summary; 
...
WITH top_3_genre_summary AS 
          (SELECT g.genre,
		         ...
		         CAST(REPLACE(REPLACE(TRIM(m.worlwide_gross_income), '$', ''), ',', '') AS DECIMAL) AS worldwide_gross_income,
		         RANK() OVER (PARTITION BY m.year ORDER BY CAST(REPLACE(REPLACE(TRIM(m.worlwide_gross_income), '$', ''), ',', '') AS DECIMAL) DESC) AS movie_rank
                 FROM movie m
                 INNER JOIN genre g ON m.id = g.movie_id
				WHERE g.genre IN ('Drama', 'Comedy', 'Thriller'))
SELECT *
FROM top_3_genre_summary
WHERE movie_rank <= 5;
...
WITH actress_summary AS
    ...
    INNER JOIN role_mapping rm ON dm.movie_id = rm.movie_id
...
SELECT 
    ...
    ROUND(AVG(DATEDIFF(CURDATE(), m.date_published))) AS avg_inter_movie_days,
